...
class BrandController extends Controller
{
    ...
    public function store(Request $request){
        ...
        'brand_logo' => 'required',
         ...
         // $photoname = $request->file('brand_logo')->store('brand');
         $photoname = $request->file('brand_logo')->store('brand');
         Storage::move($photoname, 'brand/'.$slug.'.'.$photo->getClientOriginalExtension());
         $data ['brand_logo'] = 'brand/'.$slug.'.'.$photo->getClientOriginalExtension();
    ...
    public function update(Request $request){
        ...
        if ($request->brand_logo) {
            if(Storage::exists($request->old_logo)) {
            ...
            $img = explode('.', $request->old_logo);
            Storage::move($request->old_logo, 'brand/'.$slug.'.'.$img[1]);
            $data ['brand_logo'] = 'brand/'.$slug.'.'.$img[1];
            $data ['updated_at'] = Carbon::now();
            DB::table('brands')->where('id', $id)->update($data);
            $notify = array('messege' => 'Brand Update Sucessfull !', 'alert-type' => 'success');
            return redirect()->back()->with($notify);
